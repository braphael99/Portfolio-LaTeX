...
\begin{proof}
	At line 2 we have a loop initialization with a single assignment of time complexity 1.\\ 
	The loop takes $n$ iterations, and at each iteration we have the following:\\
	\begin{itemize}[]\item Comparison $i \leq n$ which takes 1 step.
	\item Variable update which involves computation $2 \cdot i$ and assignment $i = 2 \cdot i$, giving us a time cost of 2.
	...
	\item Finally we have another loop that takes $n$ iterations and at each iteration we have the following:\\
	\begin{itemize}[]\item Comparison $j \leq n$ which takes 1 step.
		\item Variable update which involves computation $3 \cdot j$ and assignment $j = 3 \cdot j$, giving us a time cost of 2.
		\item Finally we have a \begin{verbatim} print \end{verbatim} statement having a time cost of 1.
	\end{itemize}
	\end{itemize}
So the inner loop has time complexity $1 + \displaystyle\sum_{j=1} ^{(\log_{3}n)+1} (1 + 2 + 1)$ or $1 + \displaystyle\sum_{j=1} ^{(\log_{3}n)+1} 4 = 1+ 4(\log_{3}n+1)$\\
The outer loop has time complexity $1 + \displaystyle\sum_{i=1} ^{\log_{2}n} 4i = 1 + (4\log_{2}n)(5 + 4\log_{3}n)$\\
So we have $T(n) = 1 + (4\log_{2}n)(5 + 4\log_{3}n)$ thus, $T(n) \in \Theta(\log_{3}(n))$
\end{proof}
\end{required}
